...
You can scroll backward and forward in time, in increments of the time range currently selected. For example, if your time range is set to 24 hours, scrolling backward adjusts the dashboard time range to the pervious 24-hour period.
...
Many widgets also allow you to set the number of decimal places in which you want the data displayed. 
...
== Convert line or bullet gauge widget to sparkline graph

DII allows you to convert a line/area chart or bullet graph to a non-editable sparkline graph. You can toggle back and forth between the widget as a sparkline graph and a re-sizable/editable widget.

In Sparkline mode, the widget can be moved, edited, analyzed, duplicated, exported as an image, or deleted, but it cannot be resized. 

The widget must have only a single data set. Multiple objects, multiple queries, or groupings other than “All” cannot be converted to sparkline, unless the graph is filtered down to a single object. Expressions are allowed, as long as they are performed against a single object.
