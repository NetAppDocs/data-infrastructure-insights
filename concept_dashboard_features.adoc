...
=== Custom Metrics Object Identification

An object identifier is a combination of attributes that uniquely distinguishes one object from another. 
When performing *group-by aggregations*, the system first calculates a value for each object (*rollup*) and then groups the results across all objects.

If no identifier is defined for custom metrics, multiple distinct objects may be merged, potentially leading to *inaccurate group-by results* because the system cannot separate them correctly.

*Best Practice*: Always define an object identifier when using group-by for custom metrics to ensure accurate calculations and reliable reporting.
