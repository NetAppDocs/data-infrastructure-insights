...
keywords: data collector, Promox Virtual Environment, PVE
...
[.lead] 
The DII Promox Virtual Environment collector will collect inventory and performance data for PVE based virtual machines (not LXC containers) ahd host/hypervisors. This is an HTTPS REST API-based collector that needs to speak to the Proxmox REST API provided by the pveproxy service/daemon that listens on TCP 8006.
