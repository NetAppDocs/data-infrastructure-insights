---
permalink: legal-notices.html
sidebar: sidebar
keywords:
summary:
---
= Legal notices
:icons: font
:imagesdir: ./media/

// Include the common notices
include::https://raw.githubusercontent.com/NetAppDocs/common/main/_include/common-legal-notices.adoc[]

// Include the open source notice intro
include::https://raw.githubusercontent.com/NetAppDocs/common/main/_include/open-source-notice-intro.adoc[]

// Add required link(s)


link:media/DII_ON-PREM_Notice_Nov2025.pdf[Notice for Data Infrastructure Insights (formerly Cloud Insights), window=_blank]

link:media/DII_WS_Notice_Nov2025.pdf[Notice for Workload Security (formerly Cloud Secure), window=_blank]




