...
keywords:  ransomware, simulation, simulator, restrict, attack, file tampering
summary: Test your Workload Security detection and response capabilities by simulating attacks using a provided script to validate alerts and verify policies.
---
...
[.lead]
Validate your Workload Security threat detection and automated response capabilities by simulating attacks using the included File Tampering simulation script. This allows you to test alert generation, verify policy effectiveness, and demonstrate detection capabilities in a controlled environment before encountering real threats.
...
Workload Security detects file tampering patterns based on user behavior while ONTAP ARP detects tampering activity based on encryption activities in files.
