...
Please note: FOS' default read only user "user" does NOT have view permissions on all Virtual Fabrics nor does it has "chassis role" permissions. This means you will have an exceedingly low likelihood of success using "user" with Cloud Insights, which needs to understand both the physical and logical configuration of the FOS device. 
