...
Note that the AI Assistant is currently available as a limited preview. 


=== Monitor Improvements

==== Modify a system monitor without duplicating it first

DII adds the ability to modify system monitors (sometimes called canned or default monitors). Change a monitor's name, filters, or grouping without first having to create a duplicate of the monitor.

Note that a banner will appear when viewing a changed system monitor, indicating that the monitor has been modified from its default.


==== Bulk Assignment of Health Category to Monitors

Select multiple monitors, and in the Bulk Actions menu, you can choose to exclude those monitors from Health Monitoring.
Monitors are included in Health Monitoring individually during creation, or by selecting "Update Health Monitoring" from the monitor's menu. 

==== Exclude objects from health monitor

When viewing an Infrastructure Health score, you may choose to exclude an object from health monitoring, without having to manually modify the monitor.
