...
=== AI Assistant has Asset Page Context

DII's helpful AI Assistant now knows what asset page you are on, such as Storage or VM. Using this context it suggests prompts to get you going and can understand questions you type in without referencing a specific asset, helping you analyze and investigate more efficiently.


=== Device Resolution: Pause Rules

Troubleshooting or swapping hardware? Pause Device Resolution rules to prevent automated changes from interfering with your work. Resume specific rules or all rules at once when your maintenance is complete, giving you full control over when automation resumes.

image: ahr-pause-rules.png[pausing multiple AHR rules]


=== Workload Security: Audit Events

The Workload Security link:ws_audit_events.html[Audit page] now provides events to identify changes performed by any Workload Security user in the system, including but not limited to:
