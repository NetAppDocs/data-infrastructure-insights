...
//The Data Infrastructure Insights AI Assistant is now available as a fully-supported feature for DII sites hosted on AWS. Up to 30 days of data is available to the Assistant for easy analysis and quick decision-making.

The Data Infrastructure Insights AI Assistant is now generally available to all customers for DII sites hosted on AWS

Note that because the AI Assistant accomplishes the same tasks as DII's "Infrastructure Insights" of Shared Resources Under Stress, Kubernetes Namespaces Running out of Space, and Reclaiming ONTAP Cold Storage, those features are no longer separately available in DII. The intuitive AI Assistant is ready to answer your questions.

Note also that the AI Assistant on Azure sites is still in Preview. Watch this space for announcement of general availability.
